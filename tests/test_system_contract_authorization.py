...
def test_dao_payout_requires_masternodes(client):
    ...
    with pytest.raises(AssertionError, match="Only masternodes"):
...
def test_chi_cost_update_requires_masternodes(client):
    ...
    chi_cost.set_value(new_value=7, signer="masternodes")

    assert chi_cost.current_value() == 7


def test_rewards_update_requires_masternodes(client):
    ...
    rewards.set_value(new_value=new_split, signer="masternodes")
